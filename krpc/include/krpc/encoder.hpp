...
#include <cstdint> // Додано замість старого stubs/port.h
#include <map>
#include <set>
#include <string>
#include <tuple>
...
std::string encode(int32_t value);  // Оновлено
std::string encode(int64_t value);  // Оновлено
std::string encode(uint32_t value); // Оновлено
std::string encode(uint64_t value); // Оновлено
